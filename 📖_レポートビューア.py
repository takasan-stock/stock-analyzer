...
def fetch_report_index(token: str, repo: str, branch: str) -> list:
    ...
    for item in items:
        ...
        if kind not in ("analysis", "earnings", "news"):
            continue
        results.append({
            "ticker": ticker,
            "kind": kind,
...
kind_labels = {"analysis": "📝 銘柄分析レポート", "earnings": "📊 決算分析レポート", "news": "📰 ニュース・クリップ"}
...
# ニュースクリップの場合、ソースコメント行をメタ表示して本文から除外
display_content = content
if selected_entry["kind"] == "news":
    first_line = content.split("\n")[0] if content else ""
    if first_line.startswith("<!-- source:") and first_line.endswith("-->"):
        source_name = first_line[len("<!-- source:"):][:-len("-->")].strip()
        if source_name:
            st.caption(f"📌 ソース：{source_name}")
        display_content = "\n".join(content.split("\n")[2:])
