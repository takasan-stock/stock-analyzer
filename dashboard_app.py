...
ANALYST_PROMPT_TEMPLATE = """あなたは日本株専門のヘッジファンドのチーフアナリストです。

ウォーレン・バフェットの企業分析、ピーター・リンチの成長株分析、ベンジャミン・グレアムのバリュー投資、イベントドリブンファンド、アクティビスト投資家、機関投資家、個人投資家の視点を統合し、「今後の株価がどう動く可能性があるか」を最優先に分析してください。
...
━━━━━━━━━━━━━━━━━━━━━━
以下の資料・情報を可能な限り最新情報で横断的に調査・分析してください。
...
if not _focus_df.empty:
    ...
    if _focus_items:
        ...
        if _a_items:
            ...
            st.caption("まずはAランクだけ確認。次の行動は3項目以内の短いチェックワードで表示します。")
                ...
                if _morning_opp:
                    for _item in _morning_opp:
                        ...
                        if _vr is not None and _vr >= 1.5:
                            _checks.append("出来高継続")
                        if _chg is not None and _chg >= 5:
                            _checks.append("高値更新")
                        if "材料ホット" in _tags:
                            _checks.append("材料織り込み")
                        if _ed is not None and 0 <= _ed <= 7:
                            _checks.append("決算持越し")
                        if not _checks:
                            _checks.append("押し目・出来高")
                ...
                if _morning_warn:
                    for _item in _morning_warn:
                        ...
                        if "要注意" in _tags or "損切り接近" in _tags:
                            _checks.append("損切り・保有量")
                        if _ed is not None and 0 <= _ed <= 7:
                            _checks.append("決算持越し")
                        if "急落" in _tags:
                            _checks.append("悪材料・需給")
                        if "RR低め" in _tags:
                            _checks.append("RR再確認")
                        if not _checks:
                            _checks.append("イベント・下値")
...
with tab5:
    ...
    if df.empty:
        ...
        with st.expander("🤖 AI分析用プロンプト（ヘッジファンド級・爆弾チェック付き）"):
            st.caption(
                "証券コードを埋め込んだ詳細分析プロンプトです。コピーしてChatGPT・Gemini・Claudeなど"
                "（Web検索機能つき推奨）に貼り付けてください。"
            )
            ...
            # 新プロンプトの【証券コード】欄に選択中の銘柄コードを埋め込む
            base_prompt = ANALYST_PROMPT_TEMPLATE.replace("{ticker}", f"{selected_ticker}（{company_name}）")
            ...
            if include_context and selected_row is not None:
                ...
                full_prompt = (
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"【ダッシュボード登録データ（参考）】\n{context_lines}\n"
                    f"※上記は現時点の記録値です。必ず最新のIR・TDnet・有報・決算資料を根拠に分析してください。"
                )
            else:
